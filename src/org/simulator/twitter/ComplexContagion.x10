...
(superstep :Int, aggVal :Long) => {
 if (here.id == 0){Console.OUT.println(aggVal+"  "+superstep);}

  if  (aggVal > hAmount)
  {
   if(here.id == 0)
     {Console.OUT.println("half reached at:"+(superstep-1)); return true;}
  }
  ...
  else
  {
   ...
   {
     Console.OUT.println("Peak Amount is: "+ aggVal+", on step: "+ (superstep - 1)+" but Half Amount doesn't reached.");
   }
    return true;
  }
