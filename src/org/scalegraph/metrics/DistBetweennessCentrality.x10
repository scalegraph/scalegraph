...
public class DistBetweennessCentrality implements x10.io.CustomSerialization {
    ...
    public static class LocalState {
        ...
        protected def this(dsm: DistSparseMatrix,
            ...
            // val f = new File("./result/v_" + here.id);
            // vWriter = f.printer();
        }
    }
    ...
    public static def calculate(g: Graph, directed: Boolean, normalize: Boolean): void {
        val sourceRange = 0..(g.numberOfVertices());
        _run(g, directed, normalize, -1, null, sourceRange);
    }
    
    public static def estimate(g: Graph, directed: Boolean, numSource: Long): void {
        _run(g, directed, false, numSource, null, 0..(-1 as Long));
    }
    
    public static def estimate(g: Graph, directed: Boolean, sources: Array[Vertex]): void {
        _run(g, directed, false, -1, sources, 0..(-1 as Long));
    }

    private static def _run(g: Graph, directed: Boolean, normalize: Boolean, numSource: Long, sources: Array[Vertex], sourceRange: LongRange) {
        val team = g.team();
    ...
    // public static def run(g: Graph, _sources: Array[Vertex], prefix: String) {
    //     val team = g.team();
    //     val places = team.placeGroup();
